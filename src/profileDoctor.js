...
function ProfileDoctor() {
    const { username } = useParams(); // Sử dụng username từ URL
    ...
    const [showProfile, setShowProfile] = useState(false); // Trạng thái điều khiển việc hiển thị hồ sơ
    ...
    return (
        <div>
            ...
            {!isLoggedIn ? (
                ...
                <>
                    {!showProfile ? (
                        <button onClick={() => setShowProfile(true)}>Xem hồ sơ</button>
                    ) : (
                        <div>
                            <button onClick={() => setShowProfile(false)}>Ẩn hồ sơ</button>
                            {profile && !isEditing ? (
                                <div>
                                    {imageUrl ? (
                                    ...
                                    {/* Button to navigate to add patient record page with username */}
                                    <button onClick={() => navigate(`/addpatientrecord/${profile.username}`)}>Thêm Hồ Sơ Bệnh Nhân</button>
                                </div>
                            ) : (
                                isEditing && (
